window.feta = (function(document,$){
    ...
    [
        "click",
        "focus",
        "keypress", //this needs to be worked on
    ...
    $(document).on('feta.hasRegression',function(hasRegressed){
        console.log("caught has regression event");
        if (!!hasRegressed.has){
    ...
    return{
        ...
        saveAsJS: function(report,cb,ret){
            var reportAsJS="/* TestSetup, you shouldn't need to modify this */\n";
            ...
            reportAsJS+=                "}else{";
            reportAsJS+=                    "if (res ){";
            reportAsJS+=                        "$(document).trigger('feta.hasRegression',{has: true,data: regressions});";
            reportAsJS+=                        "console.log('FETA:Regression!');";
            reportAsJS+=                    "}else{";
            reportAsJS+=                        "$(document).trigger('feta.hasRegression',{has:false});";
            ...
            reportAsJS+=    "el.val(el.val()+String.fromCharCode(keyCode));";
            reportAsJS+= "}\n\n";
            ...
            for(var i=0;i<playlist.length;i++){
                ...
                if (evt.type === "keypress"){
                    reportAsJS += "\tkeyEvent("+evt.key+",$('"+evt.elem+"'));\n";
                }else{
