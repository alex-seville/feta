window.feta = (function(document,$){
    ...
    function globalHandler(e)
    {
        if (trackingEnabled ){
            ...
            if (localStorageEnabled){
                localStorage.setItem("fetaEvents",JSON.stringify(trackedEvents));
            }
            setTimeout(function(){
    ...
    function getSelector(elem){
        ...
        if( $(elem).parents()[0] !== $("body")[0]){
            $.each($(elem).parents().get().reverse(),function() {
                ...
                var extra=$(selector).length === 1 ? "" : ":eq("+(indx)+")";
                selector+=extra+" ";
            });
        }
    ...
    return{
        ...
        start: function (useLocalStorage){
            ...
            useLocalStorage = (typeof localStorage !== 'undefined') && useLocalStorage;
            var existing = [];

            if (useLocalStorage){
                try{
                    existing = JSON.parse(localStorage.getItem("fetaEvents")) || [];
                }catch(e){
                    existing = [];
                }
            }
            ...
            if (localStorageEnabled){
                $(window).on('beforeunload', function(){
                    trackedEvents[trackedEvents.length-1].DOM=true;
                    localStorage.setItem("fetaEvents",JSON.stringify(trackedEvents));
                });
            }
        },
        stop: function(cb,ret){
            ...
            if (localStorageEnabled){
                localStorage.removeItem("fetaEvents",null);
            }

            var output="";
            for(var i=0;i<trackedEvents.length;i++){
                output += JSON.stringify(trackedEvents[i])+",";
            }
